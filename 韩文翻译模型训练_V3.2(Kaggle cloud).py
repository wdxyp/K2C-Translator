...
def train_sentencepiece_models(
    ...
    model_folder: str,
    vocab_size: int,
    ko_prefix_name: str = "spm_ko_v3_2",
    zh_prefix_name: str = "spm_zh_v3_2",
) -> tuple[str, str]:
...
def train_model_bpe(
    ...
    os.makedirs(model_folder, exist_ok=True)
    best_state_path = os.path.join(model_folder, "best_model_v3_2_bpe_attn.pth")
    best_ckpt_path = os.path.join(model_folder, "best_model_v3_2_bpe_attn.ckpt")
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for i, (src, src_len, trg, trg_len) in enumerate(train_loader):
            ...
            autocast_ctx = torch.amp.autocast("cuda", enabled=use_amp) if (device.type == "cuda" and hasattr(torch, "amp")) else (torch.cuda.amp.autocast(enabled=use_amp) if device.type == "cuda" else nullcontext())
            with autocast_ctx:
        ...
        state = {
            ...
            "ko_spm_model": os.path.join(model_folder, "spm_ko_v3_2.model"),
            "zh_spm_model": os.path.join(model_folder, "spm_zh_v3_2.model"),
    ...
    torch.save(unwrap_model(model).state_dict(), os.path.join(model_folder, f"model_v3_2_{timestamp}.pth"))
    print("训练全过程完成！", flush=True)


def resolve_default_model_dir() -> str:
    if Path("/kaggle").exists():
        return str(Path("/kaggle/working/Translate Model"))
