...
if __name__ == "__main__":
    ...
    model_path = os.path.join(model_dir, "best_model_v3_2_bpe_attn.pth")
    ko_spm_path = os.path.join(model_dir, "spm_ko_v3_2.model")
    zh_spm_path = os.path.join(model_dir, "spm_zh_v3_2.model")
    ...
    model.eval()
    print("V3.2 BPE Attention 模型加载成功！")
    ...
    while True:
        ...
        if english_terms:
            print(f"英文保留: {' '.join(english_terms)}")
        if bpe_text:
            try:
