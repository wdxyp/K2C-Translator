...
def train_sentencepiece_models(ko_sents, zh_sents, model_folder, vocab_size=16000):
    ko_prefix = os.path.join(model_folder, "spm_ko_v3_2")
    zh_prefix = os.path.join(model_folder, "spm_zh_v3_2")
...
def train_model_bpe(train_loader, test_loader, ko_sp, zh_sp, device, model_folder, max_epochs=50):
    ...
    torch.save(model.state_dict(), os.path.join(model_folder, f"model_v3_2_{timestamp}.pth"))
    print("训练全过程完成！")
...
if __name__ == "__main__":
    ...
    btn_train = tk.Button(root, text="2. 开始训练 V3.2 (BPE)", command=start_training_bpe, bg="blue", fg="white")
